...
1. 在 Visual Studio Code 中的 **16-video-indexer** 文件夹中，打开 **analyst-video.html**。这是一个基本的 HTML 页面，用于添加视频索引器播**放器**和**见解**小组件。请注意标题中对 **vb.widgets.mediator.js** 脚本的引用，此脚本使页面上的多个视频索引器小组件能够相互交互。
...
3. 请注意，免费帐户的位置为 *“试用”*。如果创建了不受限制的视频索引器帐户（具有关联的 Azure 资源），则可以将此位置更改为预配置 Azure 资源的位置（例如“美国东部”）。
...
有关**视频索引器**的详细信息，请参阅[视频索引器文档](https://docs.microsoft.com/azure/media-services/video-indexer/)。
