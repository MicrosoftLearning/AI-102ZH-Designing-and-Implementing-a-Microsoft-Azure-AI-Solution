...
5. 在创作画布中，选择“发送响应”****操作。然后，在属性窗格中，将默认文本从“- ${WelcomeUser()}”更改为**“`Hi! I'm WeatherBot.`”
6. 在创作画布中，选择最后的“+”****号（位于标记对话流结尾<u></u>的圆圈正上方），并添加新的“提问”****操作以获取文本****响应。
...
2. 在导航窗格中，选择新的“GetWeather”对话的“BeginDialog”节点********。然后在创作画布上使用“+”****号添加“提问”****操作，以获取文本****响应。
...
到目前为止，与机器人的交互都是通过文本进行的。  用户输入文本来表示其意向，而机器人使用文本进行响应。尽管文本通常是一种合适的交流方式，但可以通过其他形式的用户界面元素来增强体验。  例如，可以使用按钮来启动建议的操作，或显示一张卡片**来以可视方式呈现信息。
